...
## SCALAR TYPE
The scalar type represents a **single** value. 
These can be integers, booleans or even characters.
...
* integer overflow: when you try to change or work with a value over the range that u chose, an integer overflow happens. usually the compilor sends a "panic!" error. But when compiling with the release mode, a wrapping happens.
